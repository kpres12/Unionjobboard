...
export default function PostJobPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const { job, message } = await api.createJob(form)
      if (message) {
  ...
  return (
    <div className="mx-auto max-w-2xl">
      <h1 className="mb-8 text-3xl font-bold text-primary">Post a Job</h1>
          ...
          <div>
            ...
            {form.type === 'B-Corp' && (
              <p className="mt-2 text-sm text-muted-foreground">
                {JOB_TYPE_DESCRIPTIONS['B-Corp']} Listings may be reviewed before appearing publicly.
              </p>
            )}
          </div>
          <div>
